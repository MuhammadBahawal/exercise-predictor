...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($_POST['password'])) {
        ...
        $update_stmt->bind_param("sssddi", $new_name, $new_password, $new_role, $new_weight, $new_height, $user_id);
    } else {
        $update_sql = "UPDATE Users SET name = ?, role = ?, weight = ?, height = ? WHERE user_id = ?";
        $update_stmt = $conn->prepare($update_sql);
        $update_stmt->bind_param("ssddi", $new_name, $new_role, $new_weight, $new_height, $user_id);
    }
    ...
    <div class="container">
        ...
        <form method="POST">
            ...
            <div class="mb-3">
                ...
                <input type="number" class="form-control" name="weight" value="<?php echo $user['weight'] ?? ''; ?>"
                    step="0.1">
            </div>

            <div class="mb-3">
                <label class="form-label">Height (cm):</label>
                <input type="number" class="form-control" name="height" value="<?php echo $user['height'] ?? ''; ?>"
                    step="0.1">
            </div>
